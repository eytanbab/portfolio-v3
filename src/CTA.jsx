...
export const CTA = ({ children }) => {
  ...
  return (
    <div className='mt-8 w-full [background:linear-gradient(45deg,theme(colors.cyan.800),theme(colors.slate.950)_50%,theme(colors.slate.800))_padding-box,conic-gradient(from_var(--border-angle),theme(colors.slate.600/.48)_80%,_theme(colors.pink.500)_86%,_theme(colors.indigo.300)_90%,_theme(colors.indigo.500)_94%,_theme(colors.slate.600/.48))_border-box] rounded-full border border-transparent animate-border p-0.5'>
      <button className='w-full rounded-full py-1 bg-slate-950'>
        Learn More
      </button>
    </div>
  );
